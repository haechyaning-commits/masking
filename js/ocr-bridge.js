...
(function (global) {
   ...
   */
  async function ocrCanvasToRec(worker, canvas, ocrScale, meta) {
    const ret = await worker.recognize(canvas, {}, { blocks: true, text: false });
    let text = "";
    const charMap = [];
    const items = [];
    ...
    for (const blk of (ret.data.blocks || [])) {
      for (const par of (blk.paragraphs || [])) {
        for (const line of (par.lines || [])) {
          if (lastLine !== null) pushSep("\n");
          lastLine = line;
          const syms = [];
          for (const w of (line.words || [])) {
            const wsyms = (w.symbols && w.symbols.length) ? w.symbols : [{ text: w.text, bbox: w.bbox }];
            for (const s of wsyms) if (s.text) syms.push(s);
          }
          syms.sort((a, b) => a.bbox.x0 - b.bbox.x0);
  ...
  global.MaskOCR = { getOcrWorker, ocrCanvasToRec, idbPut, base64ToBytes };
})(window);
