...
(function (global) {
  ...
  function hasDarkRegions(canvas, ratio) {
    ...
    // 브라우저의 기본 축소 필터에 맡기면(품질 미지정) 표 테두리 같은 가는
    // 고대비 선이 다운샘플 그리드와 우연히 겹치는지에 따라 무작위로
    // 사라지거나 과대평가된다(에일리어싱) — 실측(Playwright)으로 확인: 헤더
    // 행 높이를 14/18/22/26/30/36pt로만 바꿔가며 같은 표를 렌더링했더니
    // "다크 있음/없음" 판정이 true/false/true/true/true/true로 들쭉날쭉했다.
    // 고품질(영역 평균에 가까운) 축소를 명시적으로 요청해 이 에일리어싱을
    // 없앤다.
    ...
    // 위 에일리어싱을 없애고 정확히 계산해보면(회귀 테스트), 감사보고서에
    // 흔한 "표 헤더 한 줄" 정도의 다크 영역은 A4 한 페이지 전체 면적 대비
    // 약 0.7~2%대에 그친다(예: 26pt 높이 헤더 행 ≈1.8%, 20pt 헤더 행
    // ≈0.9~1.3%) — 기존 기본값 2%는 이런 realistic한 단일 헤더 행 대부분을
    // (에일리어싱이 없어도) 애초에 놓치는 임계값이었다. 일반 본문(표
    // 테두리·글자만 있고 별도 배경색은 없는 페이지)은 정확한 계산에서 다크
    // 비율이 0%에 가까워(회귀 테스트로 확인) 임계값을 낮춰도 오탐(불필요한
    // 반전 패스 실행)이 늘지는 않는다.
    return dark / (sw * sh) >= (ratio || 0.006);
  }
