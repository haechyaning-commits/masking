...
(function () {
  ...
  // 이름·직접지정(문자 방식)에 쓸 기호 팔레트 — 같은 사람(단어)은 항상 같은 기호,
  // 다른 사람은 다른 기호로 순환 배정해 표에서 서로 다른 사람임을 구분할 수 있게 한다.
  // (번호류는 이 팔레트를 쓰지 않고 항상 사용자가 고른 state.maskChar 하나만 씀)
  ...
  const state = {
    ...
    entitySymbols: new Map(), // 사람(단어) → 배정된 기호
  };
  // key(사람/단어)에 배정된 기호를 돌려준다. 처음 보는 key면 팔레트에서 순서대로 새로 배정.
  function symbolForKey(key) {
    if (!key) return state.maskChar;
    if (state.entitySymbols.has(key)) return state.entitySymbols.get(key);
    const sym = ENTITY_SYMBOL_PALETTE[state.entitySymbols.size % ENTITY_SYMBOL_PALETTE.length];
    state.entitySymbols.set(key, sym);
    return sym;
  }
  ...
  function resetState() {
    state.pages = []; state.dets = []; detIdSeq = 0; state.entitySymbols.clear();
  ...
  async function buildRaster(included) {
    ...
    for (let n = 1; n <= pdf.numPages; n++) {
      ...
      // 이 페이지의 모든 마스킹 영역을 먼저 모은다 — 여백을 얼마나 줄 수 있는지는
      // 옆/위아래에 있는 "다른" 마스킹까지의 실제 간격을 봐야 정할 수 있다.
      const pageRects = [];
      for (const det of included) {
        ...
        if (useChar && det.category === "name") charForDet = symbolForKey(det.entityKey || det.value);
        else if (useChar && det.source === "manual" && det.style === "symbol") charForDet = symbolForKey(det.wordMatch ? det.value : det.id);
        for (const r of activeRects(det)) {
          ...
          pageRects.push({ rx, ry, rw, rh, useChar, chars: r.chars, char: charForDet });
        }
      }
      for (const cur of pageRects) {
        // 같은 줄(세로로 겹치는 다른 마스킹)까지의 가로 간격, 같은 열(가로로 겹치는
        // 다른 마스킹)까지의 세로 간격 중 가장 좁은 것을 찾는다.
        ...
        // 다만 옆/위아래에 다른 마스킹이 가까이 있으면 그 간격의 40%를 넘지 않도록
        // 상한을 둬서, 전화번호·이메일처럼 마스킹 대상끼리 붙어 있어도 서로 겹치지 않게 한다.
        ...
        if (cur.useChar) drawChars(ctx, fx, fy, fw, fh2, cur.chars, cur.char);
  ...
  function drawChars(ctx, rx, ry, rw, rh, chars, char) {
    const ch = char || state.maskChar;
