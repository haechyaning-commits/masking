...
(function () {
    ...
    for (let n = 1; n <= pdf.numPages; n++) {
      ...
      for (const m of detect(text)) {
        ...
        addDet({ pageIndex, category: m.category, source: "pii", style: "box",
          ...
          fullRange: [m.start, m.end], partialRange: [pa, pb] });
      }
    }
    return hadText;
  }
  ...
  async function extractText(page) {
    ...
    for (const item of content.items) {
      ...
      item._textStart = text.length; // 이 조각이 시작하는 페이지-텍스트 오프셋(추출 텍스트층 재구성용)
    ...
    if (el.sameWord && el.sameWord.checked && tok.token.length >= 1) {
      for (const r of state.pages) {
        ...
        while ((idx = r.text.indexOf(tok.token, from)) !== -1) {
          ...
          if (rects.length) addDet({ pageIndex: r.pageIndex, category: "manual", source: "manual", style: state.manualStyle, fullRects: rects, partialRects: rects, value: tok.token, fullRange: [idx, idx + tok.token.length], partialRange: [idx, idx + tok.token.length] });
        }
      }
    } else {
      const rect = itemSubRect(tok.item, tok.tStart, tok.tEnd);
      const g0 = tok.item._textStart || 0;
      if (rect) addDet({ pageIndex: rec.pageIndex, category: "manual", source: "manual", style: state.manualStyle, fullRects: [rect], partialRects: [rect], value: tok.token, fullRange: [g0 + tok.tStart, g0 + tok.tEnd + 1], partialRange: [g0 + tok.tStart, g0 + tok.tEnd + 1] });
    }
    renderReport();
  }
    ...
    for (const rec of state.pages) {
      for (const e of findEntities(rec.text, { words, useSuffixRule })) {
        ...
        addDet({ pageIndex: rec.pageIndex, category: "name", source: "entity", style: "symbol",
          ...
          fullRange: [e.maskStart, e.maskEnd], partialRange: [pa, pb] });
      }
    }
  }
  ...
  // 추출용 텍스트층을 지원하는 빌드(독립 실행본)면 안내 문구를 정확히 갱신
  (() => {
    if (!hasTextLayerAsset()) return;
    const note = document.getElementById("rasterNote");
    if (note) note.innerHTML = "결과는 <b>이미지화</b>로 저장되며, <b>마스킹된 개인정보는 마스킹 문자로 대체</b>되어 추출되지 않습니다. 마스킹되지 않은 본문은 <b>텍스트로 추출·검색</b>할 수 있습니다.";
  })();
    ...
    ov.addEventListener("mousedown", (e) => {
      ...
      const up = (ev) => {
        ...
        addDet({ pageIndex: rec.pageIndex, category: "manual", source: "manual", style: state.manualStyle,
          fullRects: [rect], partialRects: [rect], region: true, value: state.manualStyle === "symbol" ? "수동 (문자)" : "수동 (검은박스)" });
        renderReport();
      };
  ...
  async function buildRaster(included) {
    ...
    // 추출용 텍스트층: 내장 한글 폰트가 있는 빌드(독립 실행본)에서만 활성화
    let font = null;
    const fontBytes = maskFontBytes();
    if (fontBytes) {
      try { out.registerFontkit(window.fontkit); font = await out.embedFont(fontBytes, { subset: true }); }
      catch (e) { console.warn("텍스트층 폰트 임베드 실패 — 이미지 전용으로 저장", e); font = null; }
    }
    ...
    for (let n = 1; n <= pdf.numPages; n++) {
      ...
      const pdfPage = out.addPage([meta.widthPts, meta.heightPts]);
      pdfPage.drawImage(img, { x: 0, y: 0, width: meta.widthPts, height: meta.heightPts });
      // 이미지 위에 '보이지 않는' 텍스트층을 얹어 마스킹 완료 파일에서도 텍스트 추출/검색이 가능
      if (font) drawTextLayer(pdfPage, pageIndex, font, included);
    }
    return out.save({ updateMetadata: false }); // 위에서 비운 메타데이터 유지
  }

  // ===== 추출용(보이지 않는) 텍스트층 =====
  // 마스킹된 글자는 마스킹 문자로 대체, 나머지 본문은 원문 그대로 → 개인정보는 추출되지 않고 본문만 추출됨
  function drawTextLayer(pdfPage, pageIndex, font, included) {
    const rec = state.pages[pageIndex]; if (!rec || !rec.items || !rec.text) return;
    const masked = new Uint8Array(rec.text.length);
    for (const det of included) {
      if (det.pageIndex !== pageIndex) continue;
      if (det.region) { for (const r of det.fullRects) markRectItems(rec, r, masked); continue; }
      const range = (state.maskLevel === "partial" && det.source !== "manual") ? det.partialRange : det.fullRange;
      if (!range) continue;
      for (let i = Math.max(0, range[0]); i < Math.min(masked.length, range[1]); i++) masked[i] = 1;
    }
    for (const item of rec.items) {
      const s = item.str, t = item.transform; if (!s || !t) continue;
      const g0 = item._textStart || 0;
      let outStr = "";
      for (let k = 0; k < s.length; k++) outStr += masked[g0 + k] ? randMaskChar() : s[k];
      const safe = sanitizeForFont(font, outStr);
      if (!safe) continue;
      const size = Math.hypot(t[2], t[3]) || Math.hypot(t[0], t[1]) || 10;
      try { pdfPage.drawText(safe, { x: t[4], y: t[5], size: size, font: font, opacity: 0 }); } catch (e) {}
    }
  }
  // 드래그(영역) 수동 마스킹: 사각형과 겹치는 텍스트 조각의 글자를 모두 마스킹 처리(원문 유출 방지)
  function markRectItems(rec, r, masked) {
    for (const item of rec.items) {
      const b = itemBBox(item), ix1 = b.x0 + b.w;
      if (ix1 < r.x || b.x0 > r.x + r.w || b.top < r.y || b.bottom > r.y + r.h) continue;
      const g0 = item._textStart || 0;
      for (let k = 0; k < item.str.length; k++) if (g0 + k < masked.length) masked[g0 + k] = 1;
    }
  }
  // 마스킹 문자 하나 고르기(여러 개 선택 시 무작위로 섞음)
  function randMaskChar() {
    const pool = state.maskChars && state.maskChars.length ? state.maskChars : ["*"];
    return pool.length === 1 ? pool[0] : pool[(Math.random() * pool.length) | 0];
  }

  // ===== 내장 한글 폰트(base64) 로드 & 글리프 커버리지 =====
  let _fontBytes, _covFont; const _covCache = new Map();
  function maskFontBytes() {
    if (_fontBytes !== undefined) return _fontBytes; // 캐시(없으면 null)
    _fontBytes = null;
    try {
      const node = document.getElementById("notoKrFont");
      const b64 = node && node.textContent && node.textContent.trim();
      if (b64 && window.fontkit) {
        const bin = atob(b64);
        const arr = new Uint8Array(bin.length);
        for (let i = 0; i < bin.length; i++) arr[i] = bin.charCodeAt(i);
        _fontBytes = arr;
      }
    } catch (e) { console.warn("텍스트층 폰트 로드 실패", e); _fontBytes = null; }
    return _fontBytes;
  }
  function hasTextLayerAsset() { const n = document.getElementById("notoKrFont"); return !!(n && n.textContent && n.textContent.trim() && window.fontkit); }
  function codeCovered(cp) {
    if (_covCache.has(cp)) return _covCache.get(cp);
    let ok = true;
    try { if (!_covFont) _covFont = window.fontkit.create(maskFontBytes()); ok = _covFont.hasGlyphForCodePoint(cp); }
    catch (e) { ok = true; }
    _covCache.set(cp, ok); return ok;
  }
  // 폰트에 없는 글자는 추출 시 깨지므로 공백으로 치환(대부분 한글/영문/기호는 포함됨)
  function sanitizeForFont(font, str) {
    let out = "";
    for (const ch of str) out += codeCovered(ch.codePointAt(0)) ? ch : " ";
    return out;
  }
  ...
  function drawChars(ctx, rx, ry, rw, rh, chars) {
    ...
    for (let i = 0; i < n; i++) ctx.fillText(randMaskChar(), rx + slot * (i + 0.5), ry + rh / 2);
  }
